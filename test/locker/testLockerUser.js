...
const { getPoolParts, getTradoswapFactory} = require('../funcs')
const { getSwap, swapPointDown, swapPointUp } = require('./swap');
const { getLocker, deposit, collect, withdraw } = require("./locker.js");
...
describe("locker", function () {
    ...
    beforeEach(async function() {
        ...
        tradoswapFactory = await getTradoswapFactory(signer.address, swapX2YModule, swapY2XModule, liquidityModule, limitOrderModule, flashModule, signer);
        console.log("get tradoswapFactory");
        weth9 = await getWETH9(signer);
        console.log("get weth9");
        nflm = await getNFTLiquidityManager(tradoswapFactory.address, weth9.address);
        console.log("get nflm");
        swap = await getSwap(tradoswapFactory.address, weth9.address);
        lockTime = 7*24*60*60;
        ...
        txnNumber=  '500000000000000000000'
        const wethDepositNumber = '5000000000000000000000'
    ...
    it("check deposit", async function() {
        // lock time not enough
        expect((await locker.WEEK()).toString()).to.equal(String(lockTime))
        result = await deposit(locker, miner1, '0', lockTime - 1)
        expect(result.ok).to.equal(false)
        checkContains(result.errMsg, 'LOCKTIME MUST >= 1 WEEK');
        // try deposit other's
        result = await deposit(locker, miner1, '1', lockTime)
        expect(result.ok).to.equal(false)
        ...
        const allRelease = timeStart + lockTime * 5

        // no fees, collect fee
        const tokenList = [tokenA, tokenB, weth9]
        result = await collect(locker, miner1, '0', tokenList, weth9)
        checkStrVecEqual(result.delta.slice(0, 2), ['0', '0'])
        checkStrLT(result.delta[2], '0')
        result = await collect(locker, miner2, '1', tokenList, weth9)
        checkStrVecEqual(result.delta.slice(0, 2), ['0', '0'])
        checkStrLT(result.delta[2], '0')
        result = await collect(locker, miner1, '2', tokenList, weth9)
        checkStrVecEqual(result.delta.slice(0, 2), ['0', '0'])
        checkStrLT(result.delta[2], '0')
        result = await collect(locker, miner2, '3', tokenList, weth9)
        checkStrVecEqual(result.delta.slice(0, 2), ['0', '0'])
        checkStrLT(result.delta[2], '0')
