...
const { getPoolParts, getTradoswapFactory, newLimOrderWithX, newLimOrderWithY, decLimOrderWithY, decLimOrderWithX, collectLimOrderWithX, collectLimOrderWithY } = require("./funcs.js")
...
describe("limorder", function () {
    ...
    it("first claim first earn", async function() {
        ...
        acquireXExpect = BigNumber('300000000')
        costY = getCostY(5050, rate, acquireXExpect);
        acquireXExpect = getAcquireX(5050, rate, costY);
        seller1CostX = getCostX(5050, rate, costY)

        sellY1 = costY
        await newLimOrderWithY(0, tokenX, tokenY, seller2, limorderManager, sellY1.toFixed(0), 5050);

        await decLimOrderWithY(seller2, "0", limorderManager, "100000000000000000000");
        seller2EarnPhase1 = acquireXExpect
        ...
        await decLimOrderWithX(seller1, "0", limorderManager, "1");
        seller2EarnPhase1 = acquireXExpect
        await checkLimOrder(seller1.address, "0", limorderManager, {
            pt: 5050,
            sellingRemain: sellX1.minus(seller1CostX).minus('1'),
        ...
        const seller1Remain = sellX1.minus(seller1CostX).minus('1')
        console.log('seller1Remain: ', seller1Remain.toFixed(0))

        const acquireXExpect2 = BigNumber('1000000000').minus(acquireXExpect).minus('1')
        const earnY2 = getAcquireY(5050, rate, seller1Remain)
        const costY2 = getCostY(5050, rate, acquireXExpect2)
        console.log('costy2: ', costY2.toFixed(0));
        sellY2 = costY2.plus('20000000000')
        
        const seller2NewOrder2 = await newLimOrderWithY(0, tokenX, tokenY, seller2, limorderManager, sellY1.toFixed(0), 5050);
        expect(seller2NewOrder2).to.equal(false)
        console.log('----------------')
        const seller2NewOrder3 = await newLimOrderWithY(1, tokenX, tokenY, seller2, limorderManager, sellY2.toFixed(0), 5050);
        expect(seller2NewOrder3).to.equal(true)
        console.log('----------------')

        await decLimOrderWithX(seller1, "0", limorderManager, "1000000000000000000");

        await checkLimOrder(seller1.address, "0", limorderManager, {
            pt: 5050,
            sellingRemain: BigNumber('0'),
            sellingDec: BigNumber("1"),
            earn: costY.plus(earnY2),
            lastAccEarn: costY.plus(costY2),
            sellXEarnY: true
        });


        await decLimOrderWithY(seller2, "1", limorderManager, "1");
        seller2EarnPhase1 = acquireXExpect2
