...
describe("limorderWithSwapSwitch Y2X undesire", function () {
    ...
    it("add limorder y2x desire no offset enough", async function() {
        const nft = await nflm.liquidities('0')
        const liquidity = nft.liquidity.toString()
        console.log('liquidity: ', liquidity)
        ...
        const sqrtPrice_96_at_5050 = (await logPowMath.getSqrtPrice(5050)).toString()

        const desireXAt5050 = '1000000000000000'
        const remainYAt5050 = getCostYFromXAt(sqrtPrice_96_at_5050, desireXAt5050)
        ...
        expect(limOrder1.ok).to.equal(true)
        expect(limOrder1.amountIn).to.equal(totCostYWithFee)
        expect(limOrder1.amountOut).to.equal(acquireX5021_5051)
