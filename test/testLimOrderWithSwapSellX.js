...
describe("limorder", function () {
    ...
    it("first claim first earn", async function() {
        ...
        const seller1SellX_1 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller1,
        ...
        expect(seller1SellX_1.ok).to.equal(true)
        expect(seller1SellX_1.deltaX).to.equal(sellX1)
        expect(seller1SellX_1.deltaY).to.equal('0')

        const sellX2 = "2000000000"
        const seller2SellX_1 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller2,
        ...
        expect(seller2SellX_1.deltaX).to.equal(sellX2)
        expect(seller2SellX_1.deltaY).to.equal('0')

        const sellX3 = '3000000000'
        const seller3SellX_1 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller3,
        ...
        expect(seller3SellX_1.ok).to.equal(true)
        expect(seller3SellX_1.deltaX).to.equal(sellX3)
        expect(seller3SellX_1.deltaY).to.equal('0')

        const sellX4 = '4000000000'
        const seller4SellX_1 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller4,
        ...
        expect(seller4SellX_1.ok).to.equal(true)
        expect(seller4SellX_1.deltaX).to.equal(sellX4)
        expect(seller4SellX_1.deltaY).to.equal('0')

        let acquireXExpectAt5050 = getSum([
            sellX1,
            sellX2,
            stringDiv(sellX3, '30')
        ])

        const costYAt5050 = getCostYFromXAt((await logPowMath.getSqrtPrice(5050)).toString(), acquireXExpectAt5050)
        acquireXExpectAt5050 = getEarnXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), costYAt5050)

        const costYAt5050_WithFee = amountAddFee(costYAt5050)
        await swap.connect(trader).swapY2X(
            {
                ...
                amount: costYAt5050_WithFee,
                boundaryPt: 5051,
                maxPayed: costYAt5050_WithFee,
                minAcquired: 0,
                deadline: '0xffffffff'
            }
        );

        const sellX5 = '5000000000'
        const seller5SellX_1 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller5,
        ...
        expect(seller5SellX_1.ok).to.equal(true)
        expect(seller5SellX_1.deltaX).to.equal(sellX5)
        expect(seller5SellX_1.deltaY).to.equal('0')
        ...
        expect(collet5_1_2.deltaX).to.equal(sellX5)
        expect(collet5_1_2.deltaY).to.equal('0')

        const totSellingXAt5050 = getSum([sellX1, sellX2, sellX3, sellX4])

        const sqrtPriceAt5050 = (await logPowMath.getSqrtPrice(5050)).toString()
        const seller1EarnPhase1 = getEarnYFromXAt(sqrtPriceAt5050, sellX1)
        const seller1SoldPhase1 = sellX1
        const seller2EarnPhase1 = getEarnYFromXAt(sqrtPriceAt5050, sellX2)
        const seller2SoldPhase1 = sellX2
        const seller3EarnPhase1 = stringMinus(costYAt5050, stringAdd(seller1EarnPhase1, seller2EarnPhase1))
        const seller3SoldPhase1 = getCostXFromYAt(sqrtPriceAt5050, seller3EarnPhase1)
        ...
        expect(collect1_1_1.deltaX).to.equal('0')
        expect(collect1_1_1.deltaY).to.equal(seller1EarnPhase1)
        ...
        expect(limitOrder1.initSellingAmount).to.equal(sellX1)
        expect(limitOrder1.sellingRemain).to.equal('0')
        expect(limitOrder1.lastAccEarn).to.equal(costYAt5050)
        expect(limitOrder1.sellXEarnY).to.equal(true)
        ...
        await checkCoreEarnY(tokenX, tokenY, viewLimorder, limorderWithSwapManager, 5050, {
            earn: stringMinus(costYAt5050, seller1EarnPhase1),
            earnAssign: '0',
            sellingRemain: stringMinus(totSellingXAt5050, getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), costYAt5050)),
            sellingDesc: '0',
            lastAccEarn: costYAt5050
        });
        ...
        expect(collect2_1_1.deltaX).to.equal('0')
        expect(collect2_1_1.deltaY).to.equal(seller2EarnPhase1)
        ...
        expect(limitOrder2.initSellingAmount).to.equal(sellX2)
        expect(limitOrder2.sellingRemain).to.equal('0')
        expect(limitOrder2.lastAccEarn).to.equal(costYAt5050)
        expect(limitOrder2.sellXEarnY).to.equal(true)
        expect(limitOrder2.earn).to.equal(seller2EarnPhase1)
        expect(limitOrder2.active).to.equal(false)

        await checkCoreEarnY(tokenX, tokenY, viewLimorder, limorderWithSwapManager, 5050, {
            earn: stringMinus(costYAt5050, getSum([seller1EarnPhase1, seller2EarnPhase1])),
            earnAssign: '0',
            sellingRemain: stringMinus(totSellingXAt5050, getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), costYAt5050)),
            sellingDesc: '0',
            lastAccEarn: costYAt5050
        ...
        expect(collect3_1_1.deltaX).to.equal('0')
        expect(collect3_1_1.deltaY).to.equal(seller3EarnPhase1)
        await checkCoreEarnY(tokenX, tokenY, viewLimorder, limorderWithSwapManager, 5050, {
            earn: '0',
            earnAssign: '0',
            sellingRemain: stringMinus(totSellingXAt5050, getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), costYAt5050)),
        ...
        expect(limitOrder3.initSellingAmount).to.equal(sellX3)
        expect(limitOrder3.sellingRemain).to.equal(stringMinus(sellX3, seller3SoldPhase1))
        expect(limitOrder3.lastAccEarn).to.equal(costYAt5050)
        expect(limitOrder3.sellXEarnY).to.equal(true)
        expect(limitOrder3.earn).to.equal(seller3EarnPhase1)
        expect(limitOrder3.active).to.equal(true)

        const costYAt5050_phase2_1 = getCostYFromXAt(sqrtPriceAt5050, stringDiv(sellX3, '200'))
        const costYAt5050_phase2_2 = getCostYFromXAt(sqrtPriceAt5050, stringDiv(sellX3, '300'))
        ...
        await swap.connect(trader).swapY2X(
            {
                ...
                fee: 3000,
                recipient: trader.address,
                amount: amountAddFee(costYAt5050_phase2_1),
                boundaryPt: 5051,
                maxPayed: amountAddFee(costYAt5050_phase2_1),
                minAcquired: 0,
                deadline: '0xffffffff'
            }
        );
        ...
        await swap.connect(trader).swapY2X(
            {
                ...
                fee: 3000,
                recipient: trader.address,
                amount: amountAddFee(costYAt5050_phase2_2),
                boundaryPt: 5051,
                maxPayed: amountAddFee(costYAt5050_phase2_2),
                minAcquired: 0,
                deadline: '0xffffffff'
            }
        );
        // seller5 add after swap again
        const seller5SellX_2 = await newLimOrderWithSwap(
            0,
            tokenX,
            tokenY,
            seller5,
        ...
        expect(seller5SellX_2.ok).to.equal(true)
        expect(seller5SellX_2.deltaX).to.equal(sellX5)
        expect(seller5SellX_2.deltaY).to.equal('0')
        ...
        expect(collet5_2_1.deltaX).to.equal(sellX5)
        expect(collet5_2_1.deltaY).to.equal('0')
        ...
        const seller3EarnPhase2 = stringAdd(costYAt5050_phase2_1, costYAt5050_phase2_2)

        const accEarnPhase2 = stringAdd(costYAt5050, seller3EarnPhase2)
        ...
        expect(collect3_2_1.deltaX).to.equal('0')
        expect(collect3_2_1.deltaY).to.equal(seller3EarnPhase2)
        ...
        const soldPhase1 = getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), costYAt5050)
        const soldPhase2 = getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), seller3EarnPhase2)
        const seller3SoldPhase2 = soldPhase2
        await checkCoreEarnY(tokenX, tokenY, viewLimorder, limorderWithSwapManager, 5050, {
            earn: '0',
            earnAssign: '0',
            sellingRemain: stringMinus(totSellingXAt5050, getSum([soldPhase1, soldPhase2])),
            sellingDesc: '0',
        ...
        expect(limitOrder3_2.initSellingAmount).to.equal(sellX3)
        expect(limitOrder3_2.sellingRemain).to.equal(stringMinus(sellX3, getSum([seller3SoldPhase1, seller3SoldPhase2])))
        expect(limitOrder3_2.lastAccEarn).to.equal(accEarnPhase2)
        expect(limitOrder3_2.sellXEarnY).to.equal(true)
        ...
        const costYAt5050_phase3 = getCostYFromXAt(sqrtPriceAt5050, stringDiv(sellX3, '1000'))
        ...
        await swap.connect(trader).swapY2X(
            {
                ...
                fee: 3000,
                recipient: trader.address,
                amount: amountAddFee(costYAt5050_phase3),
                boundaryPt: 5051,
                maxPayed: amountAddFee(costYAt5050_phase3),
                minAcquired: 0,
                deadline: '0xffffffff'
            }
        );
        const seller3EarnPhase3 = costYAt5050_phase3
        const seller3SoldPhase3 = getCostXFromYAt((await logPowMath.getSqrtPrice(5050)).toString(), seller3EarnPhase3)
        ...
        expect(collect3_3_1.deltaX).to.equal(seller3SellingRemain)
        expect(collect3_3_1.deltaY).to.equal(costYAt5050_phase3)

        await checkCoreEarnY(tokenX, tokenY, viewLimorder, limorderWithSwapManager, 5050, {
            earn: '0',
            earnAssign: '0',
            sellingRemain: stringMinus(totSellingXAt5050, getSum([soldPhase1, soldPhase2, soldPhase3, seller3SellingRemain])),
            sellingDesc: '0',
        ...
        expect(limitOrder3_3.initSellingAmount).to.equal(sellX3)
        // selling remain will not clear
        expect(limitOrder3_3.sellingRemain).to.equal(stringMinus(sellX3, getSum([seller3SoldPhase1, seller3SoldPhase2, seller3SoldPhase3])))
        expect(limitOrder3_3.lastAccEarn).to.equal(accEarnPhase3)
        expect(limitOrder3_3.sellXEarnY).to.equal(true)
        ...
        expect(collect4_3_2.deltaX).to.equal(sellX4)
        expect(collect4_3_2.deltaY).to.equal('0')
