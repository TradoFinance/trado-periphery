...
contract LiquidityManager is Ownable, Base, ERC721Enumerable, ITradoSwapMintCallback {
    ...
    // callback data passed through TradoSwapPool#mint to the callback
    struct MintCallbackData {
        ...
        // fee amount of swap
        uint24 fee;
        // address to pay tokenX and tokenY to TradoSwapPool
        address payer;
    }
    ...
    constructor(
        address factory,
        address weth
    ) ERC721("TradoSwap Liquidity NFT", "TRADOSWAP-LIQUIDITY-NFT") Base(factory, weth) {
    }
    ...
    function createPool(address tokenX, address tokenY, uint24 fee, int24 initialPoint) external returns (address) {
        ...
        address pool = ITradoSwapFactory(factory).pool(tokenX, tokenY, fee);
        if (pool == address(0)) {
            pool = ITradoSwapFactory(factory).newPool(tokenX, tokenY, fee, initialPoint);
            return pool;
        }
        return pool;
    }
    ...
    function getLastFeeScale(address pool, bytes32 key) private view returns(uint256, uint256) {
        ...
        (, uint256 lastFeeScaleX_128, uint256 lastFeeScaleY_128, , ) = ITradoSwapPool(pool).liquidity(
            key
        );
    ...
    function _addLiquidity(MintParam memory mp) private returns(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        pool = ITradoSwapFactory(factory).pool(mp.tokenX, mp.tokenY, mp.fee);
        uint160 sqrtRate_96 = ITradoSwapPool(pool).sqrtRate_96();
        ...
        (amountX, amountY) = ITradoSwapPool(pool).mint(address(this), mp.pl, mp.pr, liquidity, 
    ...
    function addLiquidity(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        address pool = ITradoSwapFactory(factory).pool(poolMeta.tokenX, poolMeta.tokenY, poolMeta.fee);
        // uint160 sqrtRate_96 = ITradoSwapPool(pool).sqrtRate_96();
        require(pool != address(0), "P0");
        (sqrtPrice_96, currPt) = getPoolPrice(pool);
        liquidityDelta = MintMath.computeLiquidity(
            ...
            sqrtPrice_96,
            // sqrtRate_96
            ITradoSwapPool(pool).sqrtRate_96()
        );
        require(int128(liquid.liquidity) == int256(uint256(liquid.liquidity)), "LO");
        uint128 newLiquidity = liquidityDelta + liquid.liquidity;
        (amountX, amountY) = ITradoSwapPool(pool).mint(address(this), liquid.leftPt, liquid.rightPt, liquidityDelta, 
    ...
    function decLiquidity(
        ...
        (amountX, amountY) = ITradoSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, liquidDelta);
    ...
    function collect(
        ...
        require(pool != address(0), "P0");
        if (liquidity.liquidity > 0) {
            ITradoSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, 0);
        ...
        (amountX, amountY) = ITradoSwapPool(pool).collect(recipient, liquidity.leftPt, liquidity.rightPt, amountXLim, amountYLim);
