...
contract LimitOrderManager is Base, ITradoSwapAddLimOrderCallback {
    ...
    function decLimOrder(
        ...
        if (order.sellXEarnY) {
            (actualDeltaRefund, ) = ITradoSwapPool(pool).decLimOrderWithX(order.pt, actualDelta);
        } else {
            (actualDeltaRefund, ) = ITradoSwapPool(pool).decLimOrderWithY(order.pt, actualDelta);
        }
    ...
    function collectLimOrder(
        ...
        ITradoSwapPool(pool).collectLimOrder(recipient, order.pt, actualCollectDec, actualCollectEarn, order.sellXEarnY);
