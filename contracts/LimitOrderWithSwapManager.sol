...
contract LimitOrderWithSwapManager is Switch, Base, ITradoSwapAddLimOrderCallback, ITradoSwapCallback {
    ...
    // callback data passed through TradoSwapPool#addLimOrderWithX(Y) to the callback
    ...
    /// @param factory address of TradoSwapFactory
    ...
    function getEarnX(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = ITradoSwapPool(pool).userEarnX(key);
    ...
    function getEarnY(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = ITradoSwapPool(pool).userEarnY(key);
        return (lastAccEarn, earn, legacyEarn);
    }
    ...
    function _addLimOrder(
        ...
        if (addLimitOrderParam.sellXEarnY) {
            (order, acquire) = ITradoSwapPool(pool).addLimOrderWithX(
            ...
            (order, acquire) = ITradoSwapPool(pool).addLimOrderWithY(
                address(this), addLimitOrderParam.pt, addLimitOrderParam.amount,
                abi.encode(LimCallbackData({tokenX: addLimitOrderParam.tokenX, tokenY: addLimitOrderParam.tokenY, fee: addLimitOrderParam.fee, payer: msg.sender}))
            );
        }
        if (acquire > 0) {
            ITradoSwapPool(pool).collectLimOrder(addLimitOrderParam.recipient, addLimitOrderParam.pt, 0, acquire, addLimitOrderParam.sellXEarnY);
        }
    }
        ...
        if (addLimitOrderParam.sellXEarnY) {
            if (addLimitOrderParam.pt < currentPoint) {
                ...
                if (addLimitOrderParam.isDesireMode) {
                    (costX, acquireY) = ITradoSwapPool(pool).swapX2YDesireY(
                    ...
                    (costX, acquireY) = ITradoSwapPool(pool).swapX2Y(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX, 
                            fee: addLimitOrderParam.fee, 
            ...
            if (addLimitOrderParam.pt > currentPoint) {
                ...
                uint256 acquireX;
                if (addLimitOrderParam.isDesireMode) {
                    (acquireX, costY) = ITradoSwapPool(pool).swapY2XDesireX(
                    ...
                    (acquireX, costY) = ITradoSwapPool(pool).swapY2X(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt + 1,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX, 
                            fee: addLimitOrderParam.fee, 
    ...
    function newLimOrder(
        ...
        address pool = ITradoSwapFactory(factory).pool(addLimitOrderParam.tokenX, addLimitOrderParam.tokenY, addLimitOrderParam.fee);
    ...
    function assignLimOrderEarn(
        ...
        if (isEarnY) {
            actualAssign = ITradoSwapPool(pool).assignLimOrderEarnY(pt, amount, fromLegacy);
        } else {
            actualAssign = ITradoSwapPool(pool).assignLimOrderEarnX(pt, amount, fromLegacy);
        }
    }
    ...
    function _updateOrder(
        ...
        if (order.sellXEarnY) {
            (, legacyAccEarn) = ITradoSwapPool(pool).decLimOrderWithX(order.pt, 0);
        } else {
            (, legacyAccEarn) = ITradoSwapPool(pool).decLimOrderWithY(order.pt, 0);
        }
        uint128 sold;
    ...
    function cancel(
        ...
        if (actualDecrease > 0) {
            if (sellXEarnY) {
                ITradoSwapPool(pool).decLimOrderWithX(order.pt, actualDecrease);
            } else {
                ITradoSwapPool(pool).decLimOrderWithY(order.pt, actualDecrease);
            }
        }
        if (actualDecrease > 0 || earn > 0) {
            ITradoSwapPool(pool).collectLimOrder(recipient, order.pt, actualDecrease, earn, sellXEarnY);
        }
    ...
    function collect(
        ...
        if (earn > 0) {
            ITradoSwapPool(pool).collectLimOrder(recipient, order.pt, 0, earn, sellXEarnY);
        }
    ...
    function swapDesireSingle(
        ...
        if (params.tokenOut < params.tokenIn) {
            ...
            (amountOut, amountIn) = ITradoSwapPool(poolAddr).swapY2XDesireX(
            ...
            // tokenOut is tokenY
            // tokenIn is tokenX
            (amountIn, amountOut) = ITradoSwapPool(poolAddr).swapX2YDesireY(
                recipient, params.amount, -799999,
                abi.encode(SwapCallbackData({
    ...
    function swapAmountSingle(
        ...
        if (params.tokenIn < params.tokenOut) {
            // swapX2Y
            (amountIn, amountOut) = ITradoSwapPool(poolAddr).swapX2Y(
                recipient, params.amount, -799999,
                abi.encode(SwapCallbackData({
                    tokenX: params.tokenIn, 
                    fee: params.fee, 
            ...
            // swapY2X
            (amountOut, amountIn) = ITradoSwapPool(poolAddr).swapY2X(
                recipient, params.amount, 799999,
                abi.encode(SwapCallbackData({
                    tokenX: params.tokenOut, 
                    fee: params.fee, 
